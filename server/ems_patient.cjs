    ...
    if (req.file) {
      ...
      if (genAI) {
        try {
          ...
          // Gemini expects base64 without prefix if using inlineData, but checks format
          const base64Image = resizedBuffer.toString('base64');
          console.log(`[AI Verification] Base64 Image Length: ${base64Image.length} characters`);

          console.log('[AI Verification] image resized. Sending request to Gemini...');

          const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash", generationConfig: { responseMimeType: "application/json" } });

          const prompt = "You are an AI assistant helping emergency services verify accident reports. Analyze the image to determine if it depicts a real vehicle accident, fire, or medical emergency. Result should be strictly JSON format: { \"is_real\": boolean, \"reason\": \"short explanation\" }.";
          ...
          console.log('[AI Verification] Response received:', responseText);

          const aiResult = JSON.parse(responseText);
          verification_status = aiResult.is_real ? 'Verified' : 'Suspected Fake';
        ...
        console.log('[AI Verification] Skipped: No Gemini ID.');
        verification_status = 'Pending';
        verification_reason = 'AI Verification skipped (No API Key)';
      }
    } else {
